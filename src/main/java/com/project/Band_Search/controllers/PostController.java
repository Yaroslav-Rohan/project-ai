...
import com.project.Band_Search.repository.GenreRepository;
import com.project.Band_Search.repository.PostRepository;
...
//    }
    ...
    public Post addPost(@RequestBody Post post,
            /* @RequestParam("file") MultipartFile file,*/
                        @RequestParam(name = "email") String email) throws IOException {//в цій строчці з цим @RequestParam може бути проблема на фронті бо я просто хз як там на фронті і норм не можу тестанути , на простих html шаблонах все класно робить
        //Post postToCreate = new Post();
        User user = userRepository.findByEmail(email);

        // славік ссука в'єби валідацію
        if (user == null) {
            throw new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "NO USER");
        }
...
/*
        ...
        post.setUser_top_id(user.getId());
        post.setUser(user);
        //прописати валідацію
        String regex = "/image.*";
